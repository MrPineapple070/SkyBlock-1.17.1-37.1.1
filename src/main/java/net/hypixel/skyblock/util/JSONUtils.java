...
public class JSONUtils {
	...
	public static Item convertToItem(JsonElement element, String key) {
		if (element.isJsonPrimitive()) {
			String s = element.getAsString();
			return Registry.ITEM.getOptional(new ResourceLocation(s)).orElseThrow(() -> {
				return new JsonSyntaxException("Expected " + key + " to be an item, was unknown string '" + s + "'");
			});
		} else
	...
	public static <T> T convertToObject(@Nullable JsonElement element, String key, JsonDeserializationContext context, Class<? extends T> clazz) {
		if (element != null) {
	...
	public static String convertToString(JsonElement object, String key) {
	...
	@Nullable
	public static <T> T fromJson(Gson gson, String stream, Class<T> clazz) {
		return fromJson(gson, stream, clazz, false);
	}

	@Nullable
	public static <T> T fromJson(Gson gson, String stream, Class<T> clazz, boolean lenient) {
		return fromJson(gson, new StringReader(stream), clazz, lenient);
	}

	@Nullable
	@OnlyIn(Dist.CLIENT)
	public static <T> T fromJson(Gson gson, String stream, TypeToken<T> type) {
		return fromJson(gson, stream, type, false);
	}

	@Nullable
	@OnlyIn(Dist.CLIENT)
	public static <T> T fromJson(Gson gson, String stream, TypeToken<T> type, boolean lenient) {
	...
	public static boolean getAsBoolean(JsonObject object, String key, boolean fall_back) {
		return object.has(key) ? convertToBoolean(object.get(key), key) : fall_back;
	}

	public static byte getAsByte(JsonObject object, String key, byte fall_back) {
		return object.has(key) ? convertToByte(object.get(key), key) : fall_back;
	}

	public static float getAsFloat(JsonObject object, String string) {
	...
	public static float getAsFloat(JsonObject object, String string, float fall_back) {
	...
	public static int getAsInt(JsonObject object, String key, int fall_back) {
		return object.has(key) ? convertToInt(object.get(key), key) : fall_back;
	}

	public static Item getAsItem(JsonObject json, String string) {
		if (json.has(string)) {
	...
	@Nullable
	public static JsonArray getAsJsonArray(JsonObject object, String key, @Nullable JsonArray fall_back) {
	...
	public static JsonObject getAsJsonObject(JsonObject object, String key, JsonObject fall_back) {
	...
	public static long getAsLong(JsonObject object, String key, long fall_back) {
		return object.has(key) ? convertToLong(object.get(key), key) : fall_back;
	}

	public static <T> T getAsObject(JsonObject object, String key, JsonDeserializationContext context, Class<? extends T> clazz) {
		if (object.has(key)) {
	...
	public static <T> T getAsObject(JsonObject object, String key, T value, JsonDeserializationContext context, Class<? extends T> clazz) {
		return (T)(object.has(key) ? convertToObject(object.get(key), key, context, clazz) : value);
	}
	...
	public static String getAsString(JsonObject object, String key, String fall_back) {
	...
	public static boolean isArrayNode(JsonObject object, String key) {
		return !isValidNode(object, key) ? false : object.get(key).isJsonArray();
	}

	@OnlyIn(Dist.CLIENT)
	public static boolean isBooleanValue(JsonObject object, String key) {
	...
	public static boolean isStringValue(JsonObject object, String key) {
		return !isValidPrimitive(object, key) ? false : object.getAsJsonPrimitive(key).isString();
	}

	public static boolean isValidNode(JsonObject object, String key) {
		if (object == null)
			return false;
		else
	...
	public static boolean isValidPrimitive(JsonObject object, String key) {
	...
	public static JsonObject parse(String stream) {
		return parse(stream, false);
	}

	public static JsonObject parse(String stream, boolean lenient) {
